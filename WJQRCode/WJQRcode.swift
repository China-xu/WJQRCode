...
class WJQRcode: UIViewController{
    ...
    private func initPreviewLayer(){
        let barButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.add, target: self, action: #selector(self.selectPhotoFormPhotoLibrary(_:)))
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        picker.dismiss(animated: true) {
            ...
            if (array?.count)! > 0{
                ...
                let alertController = UIAlertController.init(title: "提示", message: "该图片没有包含一个二维码！", preferredStyle: .alert)
                let confirmAction = UIAlertAction(title: "确定", style: .default) { (_) in }
                alertController.addAction(confirmAction)
                self.present(alertController, animated: true, completion: nil)
            }
